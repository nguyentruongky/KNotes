...
open class LEOTextView: UITextView {
    ...
    open func setAttributeTextWithJSONString(_ jsonString: String) {
        guard let object = jsonString.data(using: String.Encoding.utf8) else { return }
        guard let jsonDict = try? JSONSerialization.jsonObject(with: object, options: .allowFragments) as! [String : AnyObject] else { return }

        guard let text = jsonDict["text"] as? String else { return }
